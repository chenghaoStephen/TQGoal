...
static const CGFloat ZSYNameLabHeight = 22;
...
- (void)initTheInterface
{
    ...
    self.layer.borderColor = [ RGB16(0xF8F8F8) CGColor];
    self.layer.borderWidth = 1.0f;
    self.layer.cornerRadius = 1.0f;
    self.clipsToBounds = TRUE;
    
    _titleName = [[UILabel alloc] initWithFrame:CGRectZero];
    self.titleName.font = [UIFont systemFontOfSize:12.0f];
    self.titleName.backgroundColor =  RGB16(0xF8F8F8);
    self.titleName.textAlignment = NSTextAlignmentCenter;
    self.titleName.textColor =  RGB16(0xA8A8A8);
    CGFloat xWidth = self.bounds.size.width;
    self.titleName.lineBreakMode = NSLineBreakByTruncatingTail;
    self.titleName.frame = CGRectMake(15, 0, xWidth-40, ZSYNameLabHeight);
    [self addSubview:self.titleName];
    
    CGRect tableFrame = CGRectMake(0, ZSYNameLabHeight, xWidth, self.bounds.size.height-ZSYNameLabHeight);
...
- (void)setHideTitle:(BOOL)hideTitle
{
    ...
    if (hideTitle) {
        _titleName.frame = CGRectMake(15, 0, self.bounds.size.width - 40, 0);
        _mainPopoverListView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height);
    }
}
...
- (void)refreshTheUserInterface
{
    ...
    {
        self.mainPopoverListView.frame = CGRectMake(0, ZSYNameLabHeight, self.mainPopoverListView.frame.size.width, self.mainPopoverListView.frame.size.height - ZSYCustomButtonHeight);
    }
...
-(void)setTableViewSelectIndexPath:(NSIndexPath*)indexPath{
    [self.mainPopoverListView selectRowAtIndexPath:indexPath animated:NO scrollPosition:UITableViewScrollPositionTop];
}
...
- (void)show
{
    ...
    switch (self.locationType) {
        case LocationTypeCenter:
            self.center = CGPointMake(keywindow.bounds.size.width/2.0f,
                                      keywindow.bounds.size.height/2.0f);
            break;
        case LocationTypeTop:
            self.center = CGPointMake(keywindow.bounds.size.width/2.0f,
                                      self.bounds.size.height/2.0f);
            break;
        case LocationTypeBottom:
            self.center = CGPointMake(keywindow.bounds.size.width/2.0f,
                                      keywindow.bounds.size.height - self.bounds.size.height/2.0f);
            break;
        default:
            self.center = CGPointMake(keywindow.bounds.size.width/2.0f,
                                      keywindow.bounds.size.height/2.0f);
